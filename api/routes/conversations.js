...
router.get('/', requirePermission('conversations:view'), async (req, res) => {
  const { status, assigned_to, search, page = 1, limit = 30 } = req.query;
...
router.get('/:id/messages', requirePermission('conversations:view'), async (req, res) => {
  ...
  const messages = await query(`SELECT m.*, u.first_name||' '||u.last_name AS sent_by_name
...
router.post('/:id/reply', requirePermission('conversations:reply'), async (req, res) => {
  ...
  const cv = await query(`SELECT cv.*, wa.phone_number_id, c.phone AS customer_phone
    FROM conversations cv
  ...
  try {
    const msgRes = await metaService.sendTextMessage(conv.phone_number_id, conv.customer_phone, content);
    const saved = await query(`INSERT INTO messages (org_id,conversation_id,wa_message_id,direction,type,content,status,sent_by)
...
router.put('/:id/assign', requirePermission('conversations:assign'), async (req, res) => {
  ...
  const result = await query(`UPDATE conversations SET assigned_to=$1, updated_at=NOW()
    WHERE id=$2 AND org_id=$3 RETURNING *`, [user_id || null, req.params.id, req.user.org_id]);
